...
const Speaking = () => {
  ...
  const analyzeSpeech = async () => {
    if (!transcript || transcript.trim().length === 0) {
      return alert(
        "No transcript available! Please record your speech first and wait for transcription."
      );
    }
    ...
    try {
      ...
      const grammarCount = errors.filter(
        (e) => e.rule?.issueType === "grammar"
      ).length;
      const spellingCount = errors.filter(
        (e) => e.rule?.issueType === "spelling"
      ).length;
      const styleCount = errors.filter(
        (e) => e.rule?.issueType === "style"
      ).length;
      ...
      <div className="p-6">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-gray-600 mb-4">
            Record your speech, get automatic transcription, and receive instant
            grammar feedback!
          </p>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 max-w-3xl mx-auto">
            <h3 className="font-semibold text-blue-800 mb-2">
              🎯 How Grammar Tracking Works:
            </h3>
            <ol className="text-sm text-blue-900 space-y-1 list-decimal list-inside">
              <li>
                <strong>Speech Recognition:</strong> Your voice is converted to
                text in real-time using browser technology
              </li>
              <li>
                <strong>Grammar Analysis:</strong> The transcript is checked
                using 35+ grammar rules + LanguageTool API
              </li>
              <li>
                <strong>Instant Feedback:</strong> Get detailed error reports
                      ...
                      return (
                        <div
                          ...
                          className={`p-3 rounded border ${
                            colors[issueType] || colors.grammar
                          }`}
                        >
